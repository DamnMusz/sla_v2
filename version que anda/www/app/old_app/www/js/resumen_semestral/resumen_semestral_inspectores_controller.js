app.controller('ResumenSemestralInspectoresController', function ($scope, $compile, ResumenSemestralService) {

    $scope.loading_semestral = false;
    $scope.loading_table_text = "Buscando per\u00edodo...";

    $scope.group_column_name = "Inspector";
    $scope.propertyName = 'inspector';
    $scope.reverse = false;
    ...
    $scope.GetSemestral = function () {
        if ($scope.filtro.inicio_semestre != 'undefined' && $scope.filtro.inicio_semestre != '') {
            ...
            ResumenSemestralService.getRealizadasSemestralInspectores($scope.filtro.inicio_semestre).then(function (res) {
    ...
    $scope.a_csv = function () {
        var keys = []; 
        keys.push({ key: 'inspector_triki', alias: 'Inspector' });
        ...
        $scope.download_as_file(csv_content,'resumen_semestral_inspectores.csv')
    }
