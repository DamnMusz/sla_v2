app.controller('ResumenSemestralController', function ($scope, $compile, ResumenSemestralService) {

    $scope.loading_semestral = false;
    $scope.loading_table_text = "Buscando per\u00edodo...";

    $scope.group_column_name = "Aseguradora";
    ...
    $scope.GetSemestral = function () {
        if ($scope.filtro.inicio_semestre != 'undefined' && $scope.filtro.inicio_semestre != '') {
            ...
            ResumenSemestralService.getRealizadasSemestral($scope.filtro.inicio_semestre).then(function (res) {
                $scope.resumen_semestral = res.data;
                $scope.resumen_semestral.map(remove_id);
                $scope.resumen_semestral.map(completar_aseguradora);
                for (var i = 0; i < $scope.resumen_semestral.length; i++) {
                    if ($scope.resumen_semestral[i].aseguradora == $scope.resumen_semestral[i].aseguradora_alias || semestre_todos_cero($scope.resumen_semestral[i])) {
    ...
    $scope.a_csv = function () {
        ...
        $scope.download_as_file(csv_content,'resumen_semestral.csv')
    }
